...
for (let i = 0; i < 24; i++) {
    ...
    var hourCol1 = $("<div></div>").addClass("col-2  p-3 text-right schedTime").attr('id', 'eventTime' + i).html("<h4>" + timeLabel + "</h4>");
    var hourCol2 = $("<input></input>").addClass(" col-9 d-flex align-top pl-2 pr-2 border-0 p-0 text-top bg-transparent").attr('id', 'scheduleEvent'+i).attr("type", "text");
...
function storeEvent () {
    for (let i = 0; i < 24; i++) {
        let hour = $('#eventTime' + i);
        let eventDescription = $('#scheduleEvent' + i).text();
        scheduledEvent[i] = [hour, eventDescription];
    };
    console.log(scheduledEvent)
    localStorage.setItem("calendarEvents", JSON.stringify(scheduledEvent));
};
